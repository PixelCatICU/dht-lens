...
pub fn decode_string(data: &[u8], i: usize) -> Result<(Vec<u8>, usize), BError> {
    if i >= data.len() || !(data[i].is_ascii_digit()) {
        return Err(BError::Invalid("invalid string bencode"));
    }
    let colon = find(data, i, b':').ok_or(BError::Invalid(": not found"))?;
    let len = std::str::from_utf8(&data[i..colon])
        .ok()
        .and_then(|s| s.parse::<usize>().ok())
        .ok_or(BError::ParseInt)?;
    let end = colon + 1 + len;
    if end > data.len() || end < colon + 1 {
        return Err(BError::OutOfRange);
    }
    Ok((data[colon + 1..end].to_vec(), end))
}

pub fn decode_int(data: &[u8], i: usize) -> Result<(i64, usize), BError> {
    if i >= data.len() || data[i] != b'i' {
        return Err(BError::Invalid("invalid int bencode"));
    }
    let end = find(data, i + 1, b'e').ok_or(BError::Invalid("e not found"))?;
    let num = std::str::from_utf8(&data[i + 1..end])
        .ok()
...
fn decode_item(data: &[u8], i: usize) -> Result<(BVal, usize), BError> {
    ...
    match data[i] {
        b'l' => {
            let (v, j) = decode_list(data, i)?;
            Ok((BVal::List(v), j))
        }
        b'd' => {
            let (m, j) = decode_dict(data, i)?;
            Ok((BVal::Dict(m), j))
        }
        b'i' => {
...
pub fn decode_list(data: &[u8], mut i: usize) -> Result<(Vec<BVal>, usize), BError> {
    if i >= data.len() || data[i] != b'l' {
        return Err(BError::Invalid("invalid list bencode"));
    }
    i += 1;
    let mut out = Vec::new();
    while i < data.len() {
...
pub fn decode_dict(data: &[u8], mut i: usize) -> Result<(BTreeMap<String, BVal>, usize), BError> {
    if i >= data.len() || data[i] != b'd' {
        return Err(BError::Invalid("invalid dict bencode"));
    }
    i += 1;
    ...
    while i < data.len() {
        ...
        if !data[i].is_ascii_digit() {
            return Err(BError::Invalid("invalid dict key"));
        }
        let (k, j) = decode_string(data, i)?;
        i = j;
        if i >= data.len() {
            return Err(BError::OutOfRange);
        }
        let (v, j2) = decode_item(data, i)?;
        i = j2;
...
pub fn decode(data: &[u8]) -> Result<BVal, BError> {
    let (v, _) = decode_item(data, 0)?;
    Ok(v)
}
...
pub fn encode_list(items: &[BVal]) -> Vec<u8> {
    let mut buf = Vec::new();
    buf.push(b'l');
    for it in items {
        encode_item_to(&mut buf, it);
    }
    buf.push(b'e');
    buf
}
pub fn encode_dict(m: &BTreeMap<String, BVal>) -> Vec<u8> {
    let mut buf = Vec::new();
    buf.push(b'd');
    for (k, v) in m {
...
pub fn encode_item_to(buf: &mut Vec<u8>, v: &BVal) {
    match v {
        BVal::Bytes(b) => encode_bytes(buf, b),
        BVal::Int(n) => encode_int(buf, *n),
        BVal::List(l) => {
            buf.push(b'l');
            for it in l {
                encode_item_to(buf, it);
            }
            buf.push(b'e');
        }
        BVal::Dict(m) => {
