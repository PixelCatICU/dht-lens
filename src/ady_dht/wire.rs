...
use crate::ady_dht::bencode::{self as be, BVal};
...
const HANDSHAKE_PREFIX: [u8; 28] = [
    ...
    b'o', b'c', b'o', b'l', 0, 0, 0, 0, 0, 16, 0, 1, // reserved with extension + DHT bits
];
...
impl Wire {
    ...
    pub fn response(&self) -> broadcast::Receiver<Response> {
...
impl WireRunner {
    ...
    pub fn new_with_timeout(
        ...
        let handle = WireHandle {
            req_tx: req_tx.clone(),
            resp_rx: wire.response(),
            peer_rx: wire.peer_events(),
        };
        (
            Self {
                wire,
    ...
    pub async fn run(mut self) {
        ...
        while let Some(r) = self.req_rx.recv().await {
            ...
            if r.info_hash.len() != 20
                || self.wire.blacklist.contains(&r.ip, Some(r.port))
                || self.wire.queue.contains_key(&key)
            {
                continue;
            }
            ...
            let request_timeout = self.wire.request_timeout;
...
impl WireHandle {
    ...
    pub fn subscribe(&self) -> broadcast::Receiver<Response> {
        self.resp_rx.resubscribe()
    }
    pub fn subscribe_peers(&self) -> broadcast::Receiver<PeerEvent> {
        self.peer_rx.resubscribe()
    }
    pub fn clone_handle(&self) -> WireHandle {
        WireHandle {
            req_tx: self.req_tx.clone(),
            resp_rx: self.resp_rx.resubscribe(),
            peer_rx: self.peer_rx.resubscribe(),
        }
    }
}
...
async fn fetch_metadata(
    ...
    resp_tx: broadcast::Sender<Response>,
    peer_tx: broadcast::Sender<PeerEvent>,
    req_forward: mpsc::Sender<Request>,
    blacklist: Arc<BlackList>,
    request_timeout: Duration,
) -> Result<(), ()> {
    let addr = format!("{}:{}", r.ip, r.port);
    let stream = match timeout(request_timeout, TcpStream::connect(&addr)).await {
        Ok(Ok(s)) => s,
        _ => {
            blacklist.insert(&r.ip, Some(r.port));
            return Err(());
        }
    };
    ...
    let peer_id: [u8; 20] = crate::ady_dht::util::random_id20();
    hs[48..].copy_from_slice(&peer_id);
    if timeout(request_timeout, stream.write_all(&hs))
        .await
        .is_err()
    {
        return Err(());
    ...
    if timeout(request_timeout, stream.read_exact(&mut resp))
        .await
        .is_err()
    {
        return Err(());
    ...
    // Validate handshake
    if !(resp[0] == 19 && &resp[1..20] == b"BitTorrent protocol" && (resp[25] & 0x10) != 0) {
        return Err(());
    }

    // Send extended handshake (advertise ut_metadata and ut_pex)
    let ext_hs = be::encode(&be::dict(vec![(
        "m".into(),
        be::dict(vec![
            ("ut_metadata".into(), be::int(1)),
            ("ut_pex".into(), be::int(1)),
        ]),
    )]));
    ...
    send_message(&mut stream, &buf, request_timeout).await?;
    ...
    loop {
        let (length, payload) = match read_message(&mut stream, request_timeout).await {
            Ok(v) => v,
            Err(_) => return Err(()),
        };
        if length == 0 {
        ...
        if ext_id == 0 {
            ...
            let (m, _idx) = match be::decode_dict(rest, 0) {
                Ok(v) => v,
                Err(_) => return Err(()),
            };
            let ut_meta_id = match m.get("m").and_then(|v| match v {
                BVal::Dict(mm) => mm.get("ut_metadata"),
                _ => None,
            }) {
                Some(BVal::Int(n)) => Some(*n as i64),
                _ => None,
            };
            ut_pex_id = match m.get("m").and_then(|v| match v {
                BVal::Dict(mm) => mm.get("ut_pex"),
                _ => None,
            }) {
                Some(BVal::Int(n)) => Some(*n as i64),
                _ => None,
            };
            let meta_size = match m.get("metadata_size") {
                Some(BVal::Int(n)) => *n as i64,
                _ => return Err(()),
            } as usize;
            ...
            if let Some(ut_meta) = ut_meta_id {
                for i in 0..pieces_num {
                    let head = be::encode(&be::dict(vec![
                        ("msg_type".into(), be::int(REQUEST)),
                        ("piece".into(), be::int(i as i64)),
                    ]));
                    let mut out = Vec::with_capacity(2 + head.len());
                    out.push(EXTENDED);
                    out.push(ut_meta as u8);
                    out.extend_from_slice(&head);
                    let _ = send_message(&mut stream, &out, request_timeout).await;
                }
            }
            continue;
        }
        ...
        let (d, index) = match be::decode_dict(rest, 0) {
            Ok(v) => v,
            Err(_) => return Err(()),
        };
        let dict = d;
        if let Some(BVal::Int(mt)) = dict.get("msg_type") {
            if *mt == DATA {
                let piece = match dict.get("piece") {
                    Some(BVal::Int(n)) => *n as usize,
                    _ => return Err(()),
                };
                let piece_len = length as usize - 2 - index;
                // Validate piece size
                if (piece != pieces_num - 1 && piece_len != BLOCK)
                    || (piece == pieces_num - 1 && piece_len != metadata_size % BLOCK)
                {
                    return Err(());
                }
                let mut all = pieces.take().unwrap();
                if piece < all.len() {
                    all[piece] = rest[index..index + piece_len].to_vec();
                }
                ...
                if done {
                    ...
                    if digest[..] != r.info_hash {
                        return Err(());
                    }
                    let _ = resp_tx.send(Response {
                        request: r.clone(),
                        metadata_info: Arc::new(joined),
                    });
                    return Ok(());
                } else {
                    pieces = Some(all);
        ...
        // ut_pex handling
        if let Some(pex_id) = ut_pex_id {
            if (ext_id as i64) == pex_id {
                if let Ok((pex_dict, _j)) = be::decode_dict(rest, 0) {
                    if let Some(BVal::Bytes(added)) = pex_dict.get("added") {
                        ...
                        while i + 6 <= added.len() {
                            ...
                            let port = u16::from_be_bytes([added[i + 4], added[i + 5]]);
                            i += 6;
                            let ip_s = ip.to_string();
                            let _ = peer_tx.send(PeerEvent {
...
async fn read_message(
    ...
    // read length
    let _ = timeout(request_timeout, stream.read_exact(&mut len_buf))
        .await
        .map_err(|_| ())?;
    ...
    let _ = timeout(request_timeout, stream.read_exact(&mut payload))
        .await
        .map_err(|_| ())?;
    Ok((length, payload))
}
...
async fn send_message(
    ...
    match timeout(request_timeout, stream.write_all(&buf)).await {
        Ok(Ok(())) => Ok(()),
        _ => Err(()),
    }
}
