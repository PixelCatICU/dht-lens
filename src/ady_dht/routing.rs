use crate::ady_dht::bitmap::Bitmap;
use crate::ady_dht::types::Node;
use dashmap::DashMap;
...
impl KBucketInner {
    ...
    pub fn random_child_id(&self) -> [u8; 20] {
        ...
        // copy full bytes
        if full > 0 {
            out[..full].copy_from_slice(&self.prefix.as_bytes()[..full]);
        }
        ...
        if rem > 0 {
            ...
            let mut rbyte = 0u8;
            rng.fill_bytes(std::slice::from_mut(&mut rbyte));
            // keep top rem bits from prefix, randomize lower bits
            out[full] = prefix_byte | (rbyte & (!mask));
            if full + 1 < 20 {
                rng.fill_bytes(&mut out[full + 1..]);
            }
        } else {
            if full < 20 {
                rng.fill_bytes(&mut out[full..]);
            }
        }
        out
    }
...
impl RoutingNode {
    fn new(prefix: Bitmap) -> Self {
        Self {
            left: None,
            right: None,
            bucket: Some(Arc::new(Mutex::new(KBucketInner::new(prefix)))),
        }
    }
    fn child(&self, bit: u8) -> Option<&Box<RoutingNode>> {
        if bit == 0 {
            self.left.as_ref()
        } else {
            self.right.as_ref()
    ...
    fn child_mut(&mut self, bit: u8) -> Option<&mut Box<RoutingNode>> {
        if bit == 0 {
            self.left.as_mut()
        } else {
            self.right.as_mut()
    ...
    fn set_child(&mut self, bit: u8, node: RoutingNode) {
        if bit == 0 {
    ...
    fn bucket(&self) -> Option<KBucket> {
        self.bucket.clone()
    }
    fn set_bucket(&mut self, b: Option<KBucket>) {
        self.bucket = b;
    }
}
...
impl RoutingTable {
    pub fn new(k: usize) -> Self {
        ...
        if let Some(b) = table.root.bucket() {
            table.cached_buckets.push(b);
        }
        table
    }
    ...
    fn bit_of(id: &[u8; 20], index: usize) -> u8 {
        let div = index >> 3;
        let m = index & 7;
        (id[div] & (1u8 << (7 - m))) >> (7 - m)
    }
    fn split(node: &mut RoutingNode) {
        ...
        let Some(bucket_arc) = node.bucket() else {
            return;
        };
        ...
        for n in bucket.nodes.drain(..) {
            ...
            if b == 0 {
                left.bucket().unwrap().lock().unwrap().nodes.push_back(n);
            } else {
                right.bucket().unwrap().lock().unwrap().nodes.push_back(n);
            }
        }
        ...
        for prefix_len in 1..=MAX_PREFIX {
            ...
            if bucket.nodes.len() < self.k || bucket.nodes.iter().any(|n| n.id == node.id) {
                ...
                if !self
                    .cached_buckets
                    .iter()
                    .any(|b| std::ptr::eq(&**b, &*bucket_arc))
                {
                    self.cached_buckets.push(bucket_arc.clone());
                }
                return is_new;
            }
            ...
            if prefix.matches_id_prefix(&node.id.0) {
                ...
                if let Some(ref c0) = cur.left {
                    if let Some(lb) = c0.bucket.clone() {
                        self.cached_buckets.push(lb);
                    }
                }
                if let Some(ref c1) = cur.right {
                    if let Some(rb) = c1.bucket.clone() {
                        self.cached_buckets.push(rb);
                    }
                }
    ...
    pub fn neighbors(&self, limit: usize) -> Vec<Node> {
        self.cached_nodes
            .iter()
            .take(limit)
            .map(|e| e.value().clone())
    ...
    pub fn get_neighbors_compact(&self, target: &[u8; 20], size: usize) -> Vec<u8> {
        ...
        for n in list {
            ...
            if let Some(comp) =
                crate::ady_dht::util::encode_compact_ip_port(n.addr.ip(), n.addr.port())
            {
    ...
    fn remove_from_buckets(&mut self, id: &[u8; 20]) {
        ...
        for prefix_len in 1..=MAX_PREFIX {
            if let Some(next) = cur.child(Self::bit_of(id, prefix_len - 1)) {
                cur = next;
            } else {
                break;
            }
        }
        if let Some(b) = cur.bucket.clone() {
            b.lock().unwrap().replace(id);
        }
    }
    ...
    pub fn fresh_plan(
        ...
        refresh_node_num: usize,
        is_crawl_mode: bool,
    ) -> (Vec<(std::net::SocketAddr, [u8; 20])>, Vec<String>) {
        let mut tasks = Vec::new();
        let mut clear_addrs = Vec::new();
        for b in &self.cached_buckets {
            let b_guard = b.lock().unwrap();
            if b_guard.nodes.is_empty() {
                continue;
            }
            if now.duration_since(b_guard.last_changed()) < kbucket_expired_after {
                continue;
            }
            let target = b_guard.random_child_id();
            for (i, n) in b_guard.nodes.iter().enumerate() {
                if i >= refresh_node_num {
                    break;
                }
                tasks.push((n.addr, target));
                if is_crawl_mode {
                    clear_addrs.push(n.addr.to_string());
                }
            }
        }
        (tasks, clear_addrs)
    ...
    pub fn prune(&mut self, now: Instant, node_expired_after: Duration, max_nodes: usize) {
        ...
        if cur_len > max_nodes {
            ...
            for n in nodes.into_iter().take(excess) {
                self.remove_by_addr(&n.addr.to_string());
            }
        }
    }
}
